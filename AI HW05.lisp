...
;wsb lcsb rcsb esb 
;sw sl sr se
; Then the list format is: (nw nlc nrc ne wst lcst rcst est wsb lcsb rcsb esb sw sl sr se)
...
(defun make-state (nw nlc nrc ne wst lcst rcst est wsb lcsb rcsb esb sw sl sr se) 
  (list nw nlc nrc ne wst lcst rcst est wsb lcsb rcsb esb sw sl sr se))
...
; did someone win?
; this could be re-written to be a more general - right now it only works for a 4x4 board
(defun col-winner (col player board)
  (cond ((>= col *board-cols*) nil)
        ((and (equal (nth col board) (nth (+ col *board-cols*) board))
         ...
         player)
         ((and (equal (nth (+ col *board-cols*) board) (nth (+ col (* 2 *board-cols*)) board))
              (equal (nth (+ col *board-cols*) board) (nth (+ col (* 3 *board-cols*)) board))
              (equal (nth (+ col (* 3 *board-cols*)) board) player))
         player)
...
(defun row-winner (row player board)
  (cond ((>= row *board-rows*) nil)
        ((and (equal (nth (* row *board-cols*) board) (nth (+ 1 (* row *board-cols*)) board))
         ...
         player)
        ((and (equal (nth (+ 1 (* row *board-cols*)) board) (nth (+ 2 (* row *board-cols*)) board))
              (equal (nth (+ 2 (* row *board-cols*)) board) (nth (+ 3 (* row *board-cols*)) board))
              (equal (nth (+ 3 (* row *board-cols*)) board) player))
         player)
...
(defun diagonal-winner-direction (player board direction)
  (cond ((and (equal direction 'nw-to-rcsb)
              (equal (nth 0 board) (nth 5 board))
              (equal (nth 5 board) (nth 10 board))
              (equal (nth 10 board) player))
         player)
        ((and (equal direction 'nlc-to-esb)
              (equal (nth 1 board) (nth 6 board))
              (equal (nth 6 board) (nth 11 board))
              (equal (nth 11 board) player))
         player)
         ((and (equal direction 'wst-to-sr)
              (equal (nth 4 board) (nth 9 board))
              (equal (nth 9 board) (nth 14 board))
              (equal (nth 14 board) player))
         player)
         ((and (equal direction 'lcst-to-se)
              (equal (nth 5 board) (nth 10 board))
              (equal (nth 10 board) (nth 15 board))
              (equal (nth 15 board) player))
         player)
        ((and (equal direction 'nrc-to-wsb)
              (equal (nth 2 board) (nth 5 board))
              (equal (nth 5 board) (nth 8 board))
              (equal (nth 8 board) player))
         player)
         ((and (equal direction 'ne-to-lcsb)
              (equal (nth 3 board) (nth 6 board))
              (equal (nth 6 board) (nth 9 board))
              (equal (nth 9 board) player))
         player)
          ((and (equal direction 'rcst-to-sw)
              (equal (nth 3 board) (nth 6 board))
              (equal (nth 6 board) (nth 9 board))
              (equal (nth 9 board) player))
         player)
         ((and (equal direction 'est-to-sl)
              (equal (nth 3 board) (nth 6 board))
              (equal (nth 6 board) (nth 9 board))
              (equal (nth 9 board) player))
         player)
...
(defun diagonal-winner (player board)
  (or (diagonal-winner-direction player board 'nw-to-rcsb)
      (diagonal-winner-direction player board 'nlc-to-esb)
      (diagonal-winner-direction player board 'wst-to-sr)
      (diagonal-winner-direction player board 'lcst-to-se)
      (diagonal-winner-direction player board 'nrc-to-wsb)
      (diagonal-winner-direction player board 'ne-to-lcsb)
      (diagonal-winner-direction player board 'rcst-to-sw)
      (diagonal-winner-direction player board 'est-to-sl)))
